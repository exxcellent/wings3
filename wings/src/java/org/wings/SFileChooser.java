 ...
 * application.
 * <p/>
 * <p>The uploaded file is stored temporarily in the filesystem of the server with a unique name, so that uploaded files with the
 ...
 * <p/>
 * <p>The form, you add this SFileChooser to, needs to have the encoding type <code>multipart/form-data</code> set (form.{@link
 ...
 * <p/>
 * <p>You can limit the size of files to be uploaded, so it is hard to make a denial-of-service (harddisk, bandwidth) attack from
 ...
 * <p/>
 * <b>Szenario</b> Files that are too big to be uploaded are blocked very early in the upload-process (if you are curious: this is
 * done in {@link org.wings.session.MultipartRequest}). At that time, only a partial input is read, the rest is discarded to
 * thwart denial of service attacks. Since we read only part of the input, we cannot make sure, that <em>all</em> parameters are
 ...
 * <em>one</em> event is delivered to the enclosing <em>form</em>, that contains this SFileChooser.
 * <p/>
 * <p>Note, that in this case, this will <em>not</em> trigger the action listener that you might have added to the submit-button.
 * This means, that you <em>always</em> should add your action listener to the {@link SForm} ({@link
 * SForm#addActionListener(java.awt.event.ActionListener)}), <em>not</em> the submit button.
 *
...
public class SFileChooser
    ...
    /**
     * Unlike the swing filechooser that allows to match certain file-suffices, this sets the <em>mimetype</em> to be accepted.
     * This filter may be fully qualified like <code>text/html</code> or can contain a wildcard in the subtype like <code>text/
     * *</code>. Some browsers may as well accept a file-suffix wildcard as well.
     * <p/>
     * <p>In any case, you hould check the result, since you cannot assume, that the browser actually does filter. Worse, browsers
     ...
     * <p/>
     * <p>The file returned here will delete itself if you loose the reference to it and it is garbage collected to avoid filling
     ...
     * <em>not</em> be removed from the filesystem.
     *
