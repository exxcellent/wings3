...
public class WindowStateProvider {
	...
	private final transient static Log log = LogFactory
			.getLog(WindowStateProvider.class);
	...
	public static WindowStateProvider getInstance() {
		...
		WindowStateProvider pwsp = (WindowStateProvider) session
				.getProperty(Const.WINGS_SESSION_PROPERTY_WINDOW_STATE_PROVIDER);
		if (pwsp != null) {
			return pwsp;
		} else {
			...
			session
					.setProperty(
							Const.WINGS_SESSION_PROPERTY_WINDOW_STATE_PROVIDER,
							newPwsp);
			return newPwsp;
		}
	...
	public void updateWindowState() {
		Session session = SessionManager.getSession();
		RenderRequest request = (RenderRequest) session
				.getProperty(Const.WINGS_SESSION_PROPERTY_RENDER_REQUEST);
		if (request != null) {
	...
	private synchronized void notifyWindowState(WindowStateEvent e) {
		for (WindowStateListener l : listeners
				.getListeners(WindowStateListener.class))
